...
//  StrategyChoiceHelper::InstallAll ("/", "localhost/nfd/strategy/MTBFS");
  StrategyChoiceHelper::Install (cr, "/", "localhost/nfd/strategy/multicast");
  StrategyChoiceHelper::Install (erl, "/", "localhost/nfd/strategy/multicast");
  StrategyChoiceHelper::Install (erp, "/", "localhost/nfd/strategy/multicast");
  ...
  // IF + Cache Pollution
  ApplicationContainer badconsumer = consumerHelper.Install (badlf);
  badconsumer.Start (Seconds (0.0));
  badconsumer.Stop  (Seconds (100.0));



  // Interest Flooding Attack

  ApplicationContainer IFconsumer1  = consumerHelper.Install (badlf);
  IFconsumer1.Start (Seconds (0.0));
  IFconsumer1.Stop  (Seconds (100.0));

  ApplicationContainer IFconsumer2  = consumerHelper.Install (badlf);
  IFconsumer2.Start (Seconds (0.0));
  IFconsumer2.Stop  (Seconds (100.0));
