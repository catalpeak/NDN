...
NFD_REGISTER_STRATEGY (MTBFSCR);

NFD_LOG_INIT ("MTBFSCR");

const time::milliseconds MTBFSCR::RETX_SUPPRESSION_INITIAL(10);
const time::milliseconds MTBFSCR::RETX_SUPPRESSION_MAX(250);

MTBFSCR::MTBFSCR (Forwarder& forwarder, const Name& name)
  : Strategy (forwarder)
...
const Name&
MTBFSCR::getStrategyName () {
  static Name strategyName ("/localhost/nfd/strategy/MTBFSCR/%FD%03");
  return strategyName;
}

void 
MTBFSCR::afterReceiveInterest (const Face& inFace, const Interest& interest,
...
void
MTBFSCR::afterReceiveNack(const Face& inFace, const lp::Nack& nack,
                                    const shared_ptr<pit::Entry>& pitEntry)
{
...
void
MTBFSCR::beforeSatisfyInterest ( const shared_ptr< pit::Entry > &pitEntry
...
void 
MTBFSCR::afterContentStoreHit ( const shared_ptr< pit::Entry > &pitEntry
                            , const Face &inFace, const Data &data)
{
  TokenReward (&data);
  CalculateWhenInterestHit (pitEntry);
...
void
MTBFSCR::onDroppedInterest (const Face &outFace, const Interest &interest) 
{
...
void
MTBFSCR::CalculateWhenInterestHit (const shared_ptr<pit::Entry> & pitEntry) {
  for (PitQueue::iterator ientry = pitQueue.begin ();
       ientry != pitQueue.end (); ) {
...
void
MTBFSCR::DistributeTokenPeriodly () {
...
void 
MTBFSCR::TokenConsume (const Face* face) {
  for (InterfaceList::iterator ientry = interfaceList.begin ();
       ientry != interfaceList.end (); ientry++)
  {
...
void
MTBFSCR::TokenReward (const Data * data) {
...
void
MTBFSCR::afterReceiveData ( const shared_ptr <pit::Entry> &pitEntry
                          , const Face & inFace, const Data & data) {
