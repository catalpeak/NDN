...
// content-store-change.h

#ifndef CONTENT_STORE_CHANGE_H
#define CONTENT_STORE_CHANGE_H
...
MContentStoreImpl <Policy>::Add (shared_ptr <const Data> data)
{
  ...
  for (PITQUEUE::iterator ientry = pitQueue.begin ();
    ...
    if (data->getName ().toUri () == ientry->first) {
      ThisDataEentry = 1.0 - (time.GetNowTimeMS () - ientry->second) / (3.0 * TIMEOUT);
      sumEentry += 1.0;
      avg_Eentry += ThisDataEentry / sumEentry;
      var_Eentry += pow ((ThisDataEentry - avg_Eentry), 2.0) / sumEentry;
  ...
  if (ThisDataEentry < threshlodC) {
    dataProbability = ThisDataEentry;
  } else {
    dataProbability = 1.0;
  }

  ns3::ndn::Srand srand = ns3::ndn::Srand ();
  if  (srand.IfEventHappen (dataProbability) == false) {
