...
NFD_REGISTER_STRATEGY (MTBFSERHJ);

NFD_LOG_INIT ("MTBFSERHJ");

const time::milliseconds MTBFSERHJ::RETX_SUPPRESSION_INITIAL(10);
const time::milliseconds MTBFSERHJ::RETX_SUPPRESSION_MAX(250);

MTBFSERHJ::MTBFSERHJ (Forwarder& forwarder, const Name& name)
  : Strategy (forwarder)
...
const Name&
MTBFSERHJ::getStrategyName () {
  static Name strategyName ("/localhost/nfd/strategy/MTBFSERHJ/%FD%03");
  return strategyName;
}

void 
MTBFSERHJ::afterReceiveInterest (const Face& inFace, const Interest& interest,
...
void
MTBFSERHJ::afterReceiveNack(const Face& inFace, const lp::Nack& nack,
                                    const shared_ptr<pit::Entry>& pitEntry)
{
...
void
MTBFSERHJ::beforeSatisfyInterest ( const shared_ptr< pit::Entry > &pitEntry
...
void 
MTBFSERHJ::afterContentStoreHit ( const shared_ptr< pit::Entry > &pitEntry
                            , const Face &inFace, const Data &data)
{
  TokenReward (&data);
  CalculateWhenInterestHit (pitEntry);
...
void
MTBFSERHJ::onDroppedInterest (const Face &outFace, const Interest &interest) 
{
...
void
MTBFSERHJ::afterReceiveData (const shared_ptr <pit::Entry> &pitEntry,
                           const Face &inFace, const Data &data) {
...
void
MTBFSERHJ::CalculateWhenInterestHit (const shared_ptr<pit::Entry> & pitEntry) {
  for (PitQueue::iterator ientry = pitQueue.begin ();
       ientry != pitQueue.end (); ) {
...
void
MTBFSERHJ::DistributeTokenPeriodly () {
...
void 
MTBFSERHJ::TokenConsume (const Face* face) {
  for (InterfaceList::iterator ientry = interfaceList.begin ();
       ientry != interfaceList.end (); ientry++)
  {
...
void
MTBFSERHJ::TokenReward (const Data * data) {
